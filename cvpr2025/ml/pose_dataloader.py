...
class PoseCaptureDataset(Dataset):
    ...
    def __getitem__(self, idx):
        f = self.cap[idx]
        kpt_xy = torch.tensor(f["keypoints"].xy[0], dtype=torch.float32)
        kpt_conf = torch.tensor(f["keypoints"].conf[0], dtype=torch.float32)
        hist = (
            self._avg_hist(idx)
            if self.win > 1
        ...
        return kpt_xy, kpt_conf, hist


def pose_collate(batch):
    kpt_xy, kpt_conf, hist = zip(*batch)
    return (
        torch.stack(kpt_xy),
        torch.stack(kpt_conf),
        torch.stack(hist),
    )
